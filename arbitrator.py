...
class Arbitrator:
    ...
    def choose_action(self):
        """Calls deterministic og stochastic choose_action depending on value of det_choose_action"""
    ...
    def choose_action_deterministic(self):
        ...
        if winner_behavior.get_halt_request()


        return winner_behavior.get_motor_recs()  # return motor recommendation of behavior with largest weight

    def choose_action_stochastic(self):
        """Access all active behaviors from bbcon and chooses the behavior
        with largest weight. Returns motor recommendations of this behavior"""
        num = 0
        dict_behaviors = {}  # ex: {B1: [0, 0.8], B2: [0.8, 1.3], B3: [1.3, 2.0]}
        active_behaviors = self.bbcon.active_behaviors
        ...
        for key, value in dict_behaviors.items():
            if value[1] < r:  # makes sure r matches with largest possible weight range
                winner_behavior = key
        return winner_behavior.get_motor_recs()  # return motor recommendation of behavior with weight that covers r
